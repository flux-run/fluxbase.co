...
import { ChevronDown, Plus, Box, Link } from "lucide-react";
...
export function ProjectSwitcher() {
  ...
  const handleConfirmCreate = async (name: string) => {
    const currentOrgId = localStorage.getItem("current_org_id");
    if (!currentOrgId) {
       // Fallback to finding the org from the first project if possible
       const activeOrg = projects[0]?.org_id;
       if (!activeOrg) {
         toast.error("Please select an organization first.");
         return;
       }
       const newProject = await api.createProject(name, activeOrg);
       if (newProject.id) {
    ...
    const newProject = await api.createProject(name, currentOrgId);
    if (newProject.id) {
      localStorage.setItem("flux_last_project", newProject.id);
      router.push(`/project/${newProject.id}`);
      setIsOpen(false);
    ...
    <>
      <div className="flex items-center">
            ...
            <>
              ...
              <div className="absolute top-full left-0 mt-2 w-64 bg-[#111] border border-neutral-800 rounded-lg shadow-2xl z-20 overflow-hidden py-1.5 animate-in fade-in zoom-in-95 duration-200">
                ...
                <button 
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2.5 text-xs hover:bg-neutral-800 transition text-blue-500 hover:text-blue-400 font-bold"
                >
        ...
        {currentProject && (
          <button 
            onClick={() => {
              const url = `https://${currentProject.slug}.fluxbase.co`;
              navigator.clipboard.writeText(url);
              toast.success("Project URL copied");
            }}
            className="ml-1 p-1.5 hover:bg-neutral-900 rounded-md text-neutral-600 hover:text-blue-500 transition-colors"
