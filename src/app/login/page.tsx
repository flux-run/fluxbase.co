  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/email", {
        ...
        body: JSON.stringify({ mode, email, password, name, redirect: redirectTo }),
      });
    ...
    <div className="min-h-screen bg-[#0A0A0A] relative flex flex-col items-center justify-center p-8 selection:bg-blue-500/30 selection:text-white overflow-hidden">
      ...
      <main className="w-full max-w-md relative z-10 animate-in fade-in slide-in-from-bottom-8 duration-500">
        ...
        <Card className="bg-[#0D0D0D]/80 backdrop-blur-3xl border-white/[0.12] shadow-[0_0_80px_rgba(0,0,0,0.4)] relative overflow-hidden group">
          ...
          <CardHeader className="space-y-4 text-center pb-8 pt-12">
            ...
            <div className="space-y-1">
              <p className="text-[14px] text-neutral-300 font-medium tracking-tight">Replay failures. Apply fixes. Resume execution.</p>
            </div>
          </CardHeader>
          ...
          <CardContent className="space-y-6 px-10 pb-12">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] text-neutral-600 font-black uppercase tracking-widest">or</span>
              <div className="flex-1 h-px bg-white/[0.06]" />
            </div>
            ...
            <form onSubmit={handleEmailSubmit} className="space-y-3">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full h-11 rounded-lg border border-white/[0.12] bg-white/[0.05] hover:bg-white/[0.10] text-[12px] font-black text-white uppercase tracking-widest transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
        ...
        <div className="mt-12 text-center space-y-6 opacity-20 hover:opacity-100 transition-opacity duration-700">
          <p className="text-neutral-500 text-[9px] uppercase tracking-[0.3em] font-black italic">Zero logs. Zero friction. Total visibility.</p>
          <div className="flex items-center justify-center gap-8">
            <Link href="/" className="text-neutral-600 hover:text-white transition text-[10px] font-black uppercase tracking-widest">Landing Page</Link>
            <Link href="/docs" className="text-neutral-600 hover:text-white transition text-[10px] font-black uppercase tracking-widest">Docs</Link>
          </div>
        </div>
      </main>
    </div>
