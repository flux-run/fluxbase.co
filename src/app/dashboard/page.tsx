...
function DashboardContent() {
  ...
  const handleConfirmCreate = async (name: string) => {
    const orgId = localStorage.getItem("current_org_id") || session?.org_id;
    if (!orgId) {
      alert("Please select an organization first.");
      return;
    }

    const newProject = await api.createProject(name, orgId);
    if (newProject.id) {
        ...
        <main className="max-w-6xl mx-auto px-8 pt-24 pb-32 relative z-10">
          <header className="flex items-center justify-between mb-16 animate-in fade-in slide-in-from-top-4 duration-700">
            ...
            <Button 
              ...
              className="bg-white text-black hover:bg-neutral-200 transition-all font-black h-12 px-6 rounded-none uppercase text-[10px] tracking-widest gap-3 shadow-xl hover:-translate-y-1"
            >
